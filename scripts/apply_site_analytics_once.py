...
SCRIPT_TAG = '<script src="/site-analytics.js?v=20260924" defer></script>'
...
(ROOT / 'site-analytics.js').write_text(ANALYTICS_JS, encoding='utf-8')
...
for path in ROOT.rglob('*.html'):
    ...
    if rel.startswith('.github/') or rel.startswith('node_modules/'):
        continue
    if rel in {'admin.html','acesso.html','area-usuario.html','area-usuario-preview.html','plano.html','premium-pdf.html','produto.html','google0ee6c2fc7b012a02.html'}:
        continue
    text=path.read_text(encoding='utf-8',errors='ignore')
    low=text.lower()
    # Instrumenta apenas páginas públicas canônicas do domínio e nunca páginas noindex.
    if 'lucasmpc.com.br' not in low or 'rel="canonical"' not in low or 'noindex' in low:
        continue
