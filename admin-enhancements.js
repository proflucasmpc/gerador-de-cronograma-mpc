(()=> {
  ...
  function loadLargeEdictFixModule(){if(document.querySelector('script[data-mpc-large-edict-fix]'))return;const script=document.createElement('script');script.src='/admin-large-edict-fix.js?v=20260820-1';script.defer=true;script.dataset.mpcLargeEdictFix='1';document.body.appendChild(script)}
  function init(){const panel=$('#adminPanel');if(!panel)return;installFetchObserver();buildShell(panel);makeRecentSection();renderEditingBanner();installLiveUpdates();installSaveFeedback();installDestructiveGuards();loadStudyRoutineModule();loadLargeEdictFixModule();try{const msg=sessionStorage.getItem('mpcAdminLoadedPublishedPlanMessage');if(msg){sessionStorage.removeItem('mpcAdminLoadedPublishedPlanMessage');setTimeout(()=>alert(msg),300)}}catch{}}
