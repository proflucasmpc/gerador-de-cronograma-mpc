(()=>{
  ...
  function markUpdated(path){
    const items=loadRecent();const item=items.find(x=>x.path===path);
    if(item){item.name=val('#adminStudentName')||item.name;item.goal=val('#adminGoal')||item.goal;item.updatedAt=new Date().toISOString();saveRecent(items)}
  ...
  function installFetchObserver(){
    ...
    async function wrapped(...args){
      const input=args[0];let opts=args[1]||{};
      ...
      if(isPlanPost&&pendingUpdate){
        ...
        const headers=new Headers(opts.headers||{});headers.set('Content-Type','application/json');headers.set('X-Plan-Key',current.manageKey);
        response=await original.call(this,`/api/plans?id=${encodeURIComponent(id)}`,{...opts,method:'PUT',headers});updatePath=current.path;
      }else response=await original.apply(this,args);
      try{
        if(isPlanPost){response.clone().json().then(data=>{if(!response.ok)return;if(updatePath){markUpdated(updatePath);setTimeout(()=>alert('Página atualizada com sucesso. O mesmo link foi mantido.'),120)}else if(data?.path)rememberPublished(data.path,data.manageKey||'')}).catch(()=>{})}
      }catch{}
      return response;
    }
  ...
  async function updatePublished(path){
    const item=findRecent(path);if(!item?.manageKey){alert('Esta página foi publicada antes da função de gerenciamento. Publique uma nova página uma vez para habilitar futuras atualizações no mesmo link.');return}
    if(!confirm('Atualizar esta página com o cronograma que está aberto agora? O link continuará o mesmo.'))return;
    pendingUpdate={path,manageKey:item.manageKey};const button=$('#publicPageBtn');if(!button){pendingUpdate=null;alert('Não foi possível localizar o botão de publicação.');return}button.click();
  }

  async function deletePublished(path){
    const item=findRecent(path);if(!item?.manageKey){alert('Esta página foi publicada antes da função de gerenciamento e não possui chave local de exclusão.');return}
  ...
  function init(){const panel=$('#adminPanel');if(!panel)return;installFetchObserver();buildShell(panel);makeRecentSection();installLiveUpdates();installSaveFeedback();installDestructiveGuards()}
