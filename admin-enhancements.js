(()=> {
  ...
  function injectGuidanceIntoBody(opts){
    try{
      ...
      try{data.studyRoutine=JSON.parse(localStorage.getItem('mpcAdminStudyRoutineV1')||'null')}catch{}
  ...
  function installFetchObserver(){
    ...
    async function wrapped(...args){
      ...
      if(isPlanPost){
        opts=injectGuidanceIntoBody(opts);
        args[1]=opts;
      }
    ...
    wrapped.__mpcAdminWrapped=true;
  ...
  function bypassButtonLabel(){
    return captureBypassActive()?'Reativar captura neste dispositivo':'Desativar captura neste dispositivo';
  }

  function makeStatusBar(){
    const bar=document.createElement('div');
    bar.className='mpc-admin-statusbar';
    bar.innerHTML=`
      <div>
        <div class="mpc-admin-ready" id="mpcAdminReady"><i></i><span>Preencha os dados essenciais</span></div>
        <div class="mpc-admin-save-state" id="mpcAdminSaveState">Rascunho ainda não salvo nesta sessão.</div>
      </div>
      <div class="mpc-admin-actions">
        <button type="button" id="mpcQuickPreview">Visualizar como aluno</button>
        <button type="button" id="mpcCaptureBypassBtn">${bypassButtonLabel()}</button>
        <button type="button" id="mpcQuickGenerate" class="primary">Ir para gerar</button>
        <a href="${GROUP_URL}" target="_blank" rel="noopener">Grupo gratuito do gerador</a>
      </div>`;
    return bar;
  }

  const sectionDefs=[
    ['Visão geral','adminPanelTitle'],
    ['Aluno e prova','mpc-sec-aluno'],
    ['Critérios','mpc-sec-criterios'],
    ['Atividades','mpc-sec-atividades'],
    ['Simulados','mpc-sec-simulados'],
    ['Matérias','mpc-sec-materias'],
    ['Resumo','adminInsightsCard'],
    ['PDF','mpc-sec-pdf'],
    ['Gerar','mpc-sec-gerar'],
    ['Cronogramas','mpcRecentSection']
  ];

  function assignSectionIds(panel){
    $$('.admin-card',panel).forEach(card=>{
  ...
  function buildShell(panel){
    ...
    shell.className='mpc-admin-shell';
    const sidebar=document.createElement('aside');
    sidebar.className='mpc-admin-sidebar';
    sidebar.innerHTML=`<div class="mpc-admin-title">Painel Administrativo</div><div class="mpc-admin-sub">Gerador de Cronograma MPC</div><nav class="mpc-admin-nav" aria-label="Navegação administrativa">${sectionDefs.map(([label,id])=>`<button type="button" data-admin-target="${id}">${label}</button>`).join('')}</nav>`;
    const content=document.createElement('div');
    content.className='mpc-admin-content';
    ...
    if(head){
      head.insertAdjacentElement('afterend',makeDashboard());
      head.nextElementSibling.insertAdjacentElement('afterend',makeStatusBar());
    }

    $$('.mpc-admin-nav button',sidebar).forEach(btn=>btn.addEventListener('click',()=>{
      document.getElementById(btn.dataset.adminTarget)?.scrollIntoView({behavior:'smooth',block:'start'});
    }));
    ...
    $('#mpcCaptureBypassBtn')?.addEventListener('click',()=>{
      const active=captureBypassActive();
      try{
        active?localStorage.removeItem(BYPASS_KEY):localStorage.setItem(BYPASS_KEY,'1');
      }catch{}
      const btn=$('#mpcCaptureBypassBtn');
      if(btn)btn.textContent=bypassButtonLabel();
      alert(active
        ?'Captura reativada neste navegador. Abra ou atualize uma página de aluno para testar o fluxo normal.'
        :'Captura desativada neste navegador. As páginas dos alunos poderão ser revisadas aqui sem abrir a captura.'
      );
    });
  }

  function updateSummary(){
    const student=val('#adminStudentName');
    const goal=val('#adminGoal');
    const days=val('#adminPlanDays');
    ...
    if(a)a.textContent=student||'—';
    if(b)b.textContent=goal||'—';
    if(c)c.textContent=String(subjectCount);
    if(d)d.textContent=days?`${days} dias`:'—';
    const essential=student.length>=2&&goal.length>=2&&Number(days)>0&&subjectCount>0;
    const ready=$('#mpcAdminReady');
    if(ready){
      ready.classList.toggle('ready',essential);
      $('span',ready).textContent=essential?'Pronto para gerar':'Preencha os dados essenciais';
    }
  }

  function updateSaveState(ts){
    const e=$('#mpcAdminSaveState');
    if(!e)return;
    const saved=ts||localStorage.getItem(SAVE_KEY);
    e.textContent=saved?`Último rascunho salvo: ${ptDate(saved)}`:'Rascunho ainda não salvo nesta sessão.';
  }
  ...
  function installSaveFeedback(){
    ...
    btn?.addEventListener('click',()=>{
      ...
      try{localStorage.setItem(SAVE_KEY,ts)}catch{}
      setTimeout(()=>updateSaveState(ts),120);
    });
    updateSaveState();
  }
  ...
  function installDestructiveGuards(){
    document.addEventListener('click',e=>{
      ...
      if(!confirm('Confirma esta ação? Ela pode remover informações preenchidas.')){
        e.preventDefault();
        e.stopImmediatePropagation();
        return;
      }
      btn.dataset.mpcConfirmed='1';
      setTimeout(()=>delete btn.dataset.mpcConfirmed,50);
    },true);
  }

  function makeRecentSection(){
    const sec=document.createElement('section');
    sec.className='mpc-admin-recent mpc-admin-section-anchor';
    sec.id='mpcRecentSection';
    sec.innerHTML=`
      <div class="section-title">
        <div>
          <h3>Cronogramas publicados recentemente</h3>
          <p class="helper">Os links novos podem ser atualizados sem mudar de endereço ou excluídos diretamente deste navegador.</p>
        </div>
        <div class="field" style="min-width:min(320px,100%)"><input id="mpcRecentSearch" type="search" placeholder="Buscar por aluno ou concurso"></div>
      </div>
      <div class="mpc-admin-recent-list" id="mpcRecentList"></div>
      <div id="mpcPublishedActions"></div>`;
    const panel=$('#adminPanel');
    const generate=document.getElementById('mpc-sec-gerar');
    (generate?.parentElement||panel)?.appendChild(sec);
    $('#mpcRecentSearch')?.addEventListener('input',renderRecent);
    renderRecent();
  }

  function renderRecent(){
    const list=$('#mpcRecentList');
    if(!list)return;
    const q=($('#mpcRecentSearch')?.value||'').toLowerCase().trim();
    const items=loadRecent().filter(x=>!q||`${x.name} ${x.goal}`.toLowerCase().includes(q));
    if(!items.length){
      list.innerHTML='<div class="mpc-admin-empty">Nenhum cronograma publicado registrado neste navegador.</div>';
      return;
    }
  ...
  function showPublishedActions(path){
    ...
    $('[data-copy-link]',box)?.addEventListener('click',async()=>{
      try{
  ...
  function installLiveUpdates(){
    ...
    if(list)new MutationObserver(updateSummary).observe(list,{childList:true,subtree:true});
    updateSummary();
  }

  function loadStudyRoutineModule(){
    if(document.querySelector('script[data-mpc-study-routine]'))return;
    const script=document.createElement('script');
    script.src='/admin-study-routine.js?v=20260812-1';
    script.defer=true;
    script.dataset.mpcStudyRoutine='1';
