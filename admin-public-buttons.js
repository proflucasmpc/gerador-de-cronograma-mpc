(()=>{
  ...
  function restore(){if(read().length)return;const draft=readJson(DRAFT_KEY,null),state=readJson(STATE_KEY,null),source=draft?.publicPageButtons||state?.publicPageButtons;if(Array.isArray(source)&&source.length)save(source)}
  function findAnchor(){const direct=$('#adminPdfButtonEnabled')?.closest('.admin-card');if(direct)return direct;return [...document.querySelectorAll('.admin-card')].find(card=>/personaliza[cç][aã]o do pdf|pdf/i.test(card.textContent||''))||null}
  ...
  function manager(){
    let box=$('#mpcPublicButtonsManager');if(box)return box;const anchor=findAnchor();if(!anchor)return null;
