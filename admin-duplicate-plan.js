(()=>{
  ...
  function loadUiUpgrades(){
    ...
    if(!document.querySelector('script[src*="admin-final-input-normalizer.js"]')){const script=document.createElement('script');script.src='/admin-final-input-normalizer.js?v=20260823-3';script.defer=true;document.head.appendChild(script)}
    ...
    if(!document.querySelector('script[src*="admin-final-consolidation.js"]')){const script=document.createElement('script');script.src='/admin-final-consolidation.js?v=20260823-2';script.defer=true;document.head.appendChild(script)}
    ...
    if(!document.querySelector('script[src*="admin-simulation-capacity.js"]')){const script=document.createElement('script');script.src='/admin-simulation-capacity.js?v=20260823-2';script.defer=true;document.head.appendChild(script)}
  ...
  function resetPersonalDraft(draft){
    ...
    return {...draft,studentName:'',startDate:'',hoursPerDay:0,sessionMinutes:60,preferredStart:'',availableDays:[],workSchedule:'',studyMaterial:'',generalGuidance:'',replaceSchedule:true};
  }
  ...
  function convertLoadedCopy(){
    ...
    state.studentName='';state.tasks=[];state.hoursPerDay=0;state.sessionMinutes=60;state.preferredStart='';state.availableDays=[];state.startDate='';delete state.studyRoutine;state.adminGenerated=false;
    state.adminPersonalization={...(state.adminPersonalization||{}),studentName:'',startDate:'',endDate:''};write(STATE_KEY,state);
    ...
    sessionStorage.setItem('mpcAdminLoadedPublishedPlanMessage',`Nova ficha criada a partir de ${intent.name||'cronograma'}. Conteúdos do concurso foram mantidos, mas rotina, horários, carga diária, progresso e atividades do aluno anterior foram zerados. Preencha os dados do novo aluno, gere o cronograma e só então crie a nova página.`);
    location.reload();return true;
  }

  function showBanner(){
    const editing=read(EDITING_KEY,null);if(editing?.mode!=='duplicate')return;
    const box=document.getElementById('mpcEditingPlanBanner');if(!box)return;
  ...
  function addButtons(){
    document.querySelectorAll('#mpcRecentList .mpc-admin-recent-item').forEach(row=>{
      ...
      const load=row.querySelector('[data-load-plan]');if(!load)return;
      const path=load.dataset.loadPlan||'';const item=(read(RECENT_KEY,[])||[]).find(entry=>entry.path===path);
      const button=document.createElement('button');button.type='button';button.dataset.duplicatePlan=path;button.textContent='Duplicar para novo aluno';
      button.addEventListener('click',()=>{write(INTENT_KEY,{path,name:item?.name||'cronograma'});load.click()});load.insertAdjacentElement('afterend',button);
    });
  }

  function init(){
    loadUiUpgrades();if(convertLoadedCopy())return;showBanner();addButtons();
    const list=document.getElementById('mpcRecentList');if(list)new MutationObserver(addButtons).observe(list,{childList:true,subtree:true});setTimeout(showBanner,250);
  }
