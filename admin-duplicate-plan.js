(()=>{
  ...
  function loadUiUpgrades(){
    ...
    if(!document.querySelector('script[src*="admin-final-input-normalizer.js"]')){const script=document.createElement('script');script.src='/admin-final-input-normalizer.js?v=20260823-1';script.defer=true;document.head.appendChild(script)}
    ...
    if(!document.querySelector('script[src*="admin-simulation-capacity.js"]')){const script=document.createElement('script');script.src='/admin-simulation-capacity.js?v=20260823-1';script.defer=true;document.head.appendChild(script)}
  ...
  function convertLoadedCopy(){
    ...
    const draft=resetPersonalDraft(read(DRAFT_KEY,{})||{});
    ...
    state.preferredStart='';
    state.availableDays=[];
    state.startDate='';
    delete state.studyRoutine;
    state.adminGenerated=false;
    state.adminPersonalization={
      ...(state.adminPersonalization||{}),
      studentName:'',
      startDate:'',
      endDate:''
  ...
  function showBanner(){
    ...
    box.innerHTML=`<strong>Nova cópia independente — ainda não publicada</strong><span>Modelo: ${editing.sourceName||'cronograma publicado'} · rotina e disponibilidade devem ser preenchidas para o novo aluno · a próxima publicação criará um novo link.</span>`;
  }
