...
start = s.index('  function scheduleReviews(')
end = s.index('  function transformGeneratedSchedule(', start)
s = s[:start] + r'''  function scheduleReviews(items,slots,preferredStart){
    const created=[];
...
a = p2.read_text().replace('/admin-study-routine.js?v=20260812-3','/admin-study-routine.js?v=20260812-4')
p2.write_text(a)
